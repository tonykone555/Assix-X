...
const airTemplateContent = `export function buildAirTemplate(lead = {}, content = {}, nicheKey = 'hvac') {
  const companyName = lead.name || lead.companyName || lead.businessName || 'AirPro Services';
  const phone = lead.phone || '(555) 987-6543';
  const primaryColor = content.primaryColor || '#2563EB'; // Blue
  ...
  <section class="hero-section">
    \${content.model3dUrl && content.show3dHero ? \`
      <model-viewer 
        ...
        auto-rotate 
        shadow-intensity="1" 
        style="width: 100%; height: 100%; position: absolute; inset: 0; background-color: #F8FAFC;">
      </model-viewer>
    \` : \`
      <img class="hero-bg-image" src="\${heroImage}" alt="\${companyName}">
    \`}
    <div class="hero-overlay" style="\${content.model3dUrl && content.show3dHero ? 'display: none;' : ''}"></div>
...
fs.writeFileSync('services/templates/airTemplate.js', airTemplateContent);
