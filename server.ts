...
async function searchGoogleMapsForPhone(businessName: string, city: string): Promise<string | null> {
  ...
  console.log(`[Google Maps Enrichment] Spinning up temporary Stagehand session for "${businessName}" in "${city}"`);
  try {
    const { createStagehandSession } = await import('./services/browserEngine');
    const sessionRes = await createStagehandSession(tempTaskId);
    const stagehand = sessionRes.stagehand as any;
    ...
    const result = await stagehand.extract(
      "Find the phone number of the business. Look for a phone icon or a number that fits the pattern +1 XXX XXX-XXXX or similar.",
      {
        type: "object",
        properties: {
          phone: {
            type: "string",
            description: "The phone number of the business or null if not found"
          }
        }
      }
    );
    ...
    const phone = result && typeof result === 'object' ? (result as any).phone : null;
