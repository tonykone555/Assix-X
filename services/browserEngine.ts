...
export async function createStagehandSession(taskId: string) {
  ...
  // 0. Primary Engine: Steel Cloud Browser Engine (if STEEL_API_KEY set)
  if (process.env.STEEL_API_KEY) {
    try {
      console.log(`[browserEngine] Initializing Steel cloud browser session for task ${taskId}...`);
      ...
      if (!session.websocketUrl) {
        throw new Error("Steel session created but websocketUrl is missing.");
      }
      const browser = await chromium.connectOverCDP(session.websocketUrl, { timeout: 30000 });
      ...
      const liveViewerUrl = session.sessionViewerUrl || session.debugUrl || (session.id ? `https://app.steel.dev/sessions/${session.id}` : undefined);
      activeSessions.set(taskId, { browser, context, page, createdAt: Date.now(), liveViewUrl: liveViewerUrl });
      console.log(`[browserEngine] Steel cloud browser session created successfully. Live viewer: ${liveViewerUrl}`);
      return { page, sessionId: taskId, liveViewUrl: liveViewerUrl };
    } catch (steelErr: any) {
      console.warn(`[browserEngine] Steel cloud browser session launch failed: ${steelErr.message}. Falling back seamlessly to local Playwright browser...`);
    }
  }
  ...
  try {
    ...
    try {
      ...
      if (launchErr.message?.includes("Executable doesn't exist") || launchErr.message?.includes("playwright install")) {
        console.log(`[browserEngine] Playwright browser executable missing. Auto-installing Chromium binaries...`);
        try {
          execSync('npx playwright install chromium', { stdio: 'pipe' });
          console.log(`[browserEngine] Chromium installed successfully. Retrying launch...`);
          browser = await chromium.launch({
            headless: true,
            args: ['--no-sandbox', '--disable-setuid-sandbox', '--disable-dev-shm-usage', '--disable-gpu'],
            timeout: 60000,
          ...
          throw new Error(`Failed to auto-install Playwright chromium: ${installErr.message}`);
        }
      } else {
        throw launchErr;
      }
