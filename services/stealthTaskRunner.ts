...
export async function runStealthAutomation(
  ...
  try {
    while (stepCount < maxSteps && !done) {
      ...
      let promptContent = `You are UI-TARS / Agent TARS Vision-First Hybrid Controller.
Task: "${instruction}"
...
`;
      ...
      promptContent += `Step ${stepCount}/${maxSteps}.\n` +
        `Page text excerpt: ${pageText}\n` +
        `Visually analyze the attached screenshot (70% priority) to understand page layout, buttons, cards, popups, and inputs.\n` +
        `Decide the single next action. Respond ONLY with JSON:\n` +
        `{"action": "click"|"type"|"stuck"|"done", "selector": "css selector or button text", "text": "text to type if needed", "reasoning": "vision + DOM strategy reasoning", "obstacle": "description of what's blocking, only if action is stuck"}\n` +
      ...
      let strategy = 'HYBRID';
      if (decision.selector && /^(button|input|#|\.|\/|data-agent-id)/i.test(decision.selector)) {
        strategy = 'DOM';
      } else if (decision.reasoning && /visual|screenshot|screen|image|pixel|coordinate/i.test(decision.reasoning)) {
        strategy = 'VISION';
      }

      onProgress({ step: "action", status: "running", data: { message: `[${strategy}] Step ${stepCount}: ${decision.action} — ${decision.reasoning}`, strategy } });
