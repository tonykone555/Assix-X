...
export async function callGemini(contents: any[], systemInstruction?: string, isJson?: boolean, enableSearch?: boolean): Promise<string> {
  ...
  for (const modelName of modelsToTry) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        console.log(`[Gemini API] Querying ${modelName} (attempt ${attempt}/${maxRetries}, search=${!!enableSearch})...`);
        const config: any = {
        ...
        if (isQuotaExceeded || isNotFound) {
          console.warn(`[Gemini API] ${modelName} returned ${isNotFound ? '404 NOT_FOUND' : 'Quota Exceeded'}. Moving immediately to the next fallback model.`);
...
*Our local browser driver will boot, dismiss any cookie consent GDPR popups, type the query, explicitly press the 'Enter' key, scroll the feed, and extract rich local leads directly to your database!*`;
  ...
  if (taskType === "lead_enrichment") {
    ...
    const urls = Array.from(new Set(combinedContext.match(urlRegex) || [])).filter(u => !u.includes('google.com') && !u.includes('societe.com') && !u.includes('pagede.com') && !u.includes('annuaire') && !u.includes('duckduckgo.com'));
  ...
  // 3. SITE GENERATION / LANDING PAGE JSON FALLBACK
  if (taskType === "site_generation" || lower.includes("landing page json") || lower.includes("schema without markdown") || lower.includes("layoutorder")) {
    const companyNameMatch = combinedContext.match(/Name:\s*"([^"]+)"/i) || combinedContext.match(/Business Details:[\s\S]*?Name:\s*"([^"]+)"/i);
    const companyName = companyNameMatch ? companyNameMatch[1] : "Entreprise";
    const cityMatch = combinedContext.match(/Location\/City:\s*"([^"]+)"/i) || combinedContext.match(/City:\s*"([^"]+)"/i);
    ...
    return JSON.stringify({
      ...
      heroTitle: `${companyName} - Services professionnels ${city ? `à ${city}` : ''}`.trim(),
      heroSubtitle: `Rapidité, transparence et qualité garantie pour tous vos travaux et prestations ${city ? `à ${city}` : ''}.`.trim(),
      tagline: "Service Professionnel Garanti 24/7",
      ctaButton: "Demander un Devis Gratuit",
      aboutTitle: `À propos de ${companyName}`,
      aboutText: `${companyName} est votre partenaire de confiance ${city ? `à ${city}` : ''}. Nous mettons notre savoir-faire au service de vos projets avec réactivité et professionnalisme.`,
      aboutHighlights: [
      ...
      stats: [
        ...
        { "value": "24/7", "label": "Assistance Téléphonique" },
        { "value": "500+", "label": "Projets Réalisés" }
      ],
      services: [
        { "title": "Prestation Principale", "description": "Intervention complète avec diagnostic et réalisation sur-mesure.", "price": "Sur devis" },
        { "title": "Entretien & Maintenance", "description": "Suivi régulier et optimisation pour garantir la durabilité.", "price": "Forfait fixe" },
        { "title": "Service d'Urgence", "description": "Prise en charge prioritaire pour résoudre vos imprévus immédiatement.", "price": "24h/24" }
      ],
      portfolio: [
        { "title": "Réalisation Moderne", "category": "Projet" },
        { "title": "Chantier Récent", "category": "Exécution" },
        { "title": "Service Sur-Mesure", "category": "Finition" }
      ],
      whyUs: [
        { "title": "Expertise Reconnue", "text": "Des professionnels formés aux dernières normes du secteur." },
        { "title": "Réactivité Maximale", "text": "Un devis clair et rapide transmis dès votre première demande." },
        { "title": "Devis 100% Gratuit", "text": "Aucun engagement avant la validation détaillée de votre projet." }
      ],
      steps: [
        { "step": "01", "title": "Contact", "description": "Demande de devis en ligne ou par téléphone." },
        { "step": "02", "title": "Estimation", "description": "Évaluation gratuite et proposition détaillée." },
        { "step": "03", "title": "Réalisation", "description": "Prestation soignée respectant vos délais." }
      ],
      testimonials: [
        { "name": "Jean D.", "text": "Super service, très professionnel et réactif !", "rating": 5, "city": city || "Paris" }
      ],
      faq: [
        { "question": "Comment obtenir un devis ?", "answer": "Remplissez le formulaire en ligne ou contactez-nous directement par téléphone pour une estimation gratuite." },
        { "question": "Quels sont vos délais d'intervention ?", "answer": "Nous intervenons généralement sous 24h à 48h selon la nature de votre demande." }
      ]
    });
  }

  // 4. REPORT / STRATEGY GENERATION FALLBACK
