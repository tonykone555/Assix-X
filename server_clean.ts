...
const cleanPhoneString = (raw: string): string => {
  ...
  str = str.replace(/(?:Phone\s*Number|Téléphone\s*fixe|Téléphone|Telephone|Phone|Call|Appeler|Appel|Tél|Tel|Numéro|Mobile|Fixe)(?:\s*(?:au|le|du|de|:))?\s*/gi, '');
...
const formatPhone = (raw: string, countryOrCity?: string, address?: string): string => {
  ...
  const deKeywords = ['germany', 'deutschland', 'german', 'berlin', 'munich', 'münchen', 'hamburg', 'frankfurt', 'cologne', 'köln', 'stuttgart', 'de', '49'];
  const ukKeywords = ['united kingdom', 'uk', 'england', 'scotland', 'wales', 'london', 'manchester', 'birmingham', 'leeds', 'glasgow', 'gb'];
  const esKeywords = ['spain', 'españa', 'madrid', 'barcelona', 'valencia', 'es'];
  const itKeywords = ['italy', 'italia', 'rome', 'roma', 'milan', 'milano', 'it'];
  const chKeywords = ['switzerland', 'suisse', 'schweiz', 'zurich', 'zürich', 'geneva', 'genève', 'ch'];
  ...
  if (taskId) {
    ...
    try {
      ...
      if (sessionRes?.liveViewUrl) {
        await logAction(taskId, `⚡ Steel Cloud Browser session initialized! Live Viewer: ${sessionRes.liveViewUrl}`, 'success');
        await db.collection('assix_tasks').doc(taskId).update({
...
const runGoogleMapsScrape = async (taskId: string, config: any) => {
  try {
    ...
    io.emit('task_update', {
      ...
      status: 'running',
      step: 'discovering',
      message: `🚀 Querying Google Maps Search for "${cleanNiche}" in "${city}"...`
    });
...
const runWhatsAppOutreach = async (taskId: string, config: any) => {
  ...
  try {
    ...
    for (let i = 0; i < targets.length; i++) {
      ...
      if (res.success) {
        await logAction(taskId, `✓ Message delivered to ${phone} via Baileys`, 'success');
        sent++;
      } else {
        await logAction(taskId, `✕ Delivery error to ${phone}: ${res.error}`, 'warning');
      }
    ...
    await logAction(taskId, `✓ Baileys WhatsApp automation complete. Sent: ${sent}/${targets.length}`, 'success');
...
Return ONLY the 3 lines separated by line breaks (no headers or chatter).`;
        ...
        foundUniqueness = `• Specialized B2B service provider established in ${searchLoc || 'France'}.\n• High-quality tailored solutions customized to client requirements.\n• Proven local track record with fast turnaround times and verified standards.`;
      }
    }
...
const GLOBAL_GOV_REGISTRIES: Record<string, any> = {
  FR: {
    ...
    sources: [
      ...
      },
      {
        ...
        authority: "Ministère de la Justice & Trade Courts",
    ...
    categories: [
      {
        ...
        icon: "Building2",
        items: [
          { id: "agences_immo", name: "Agences Immobilières", codeNaf: "68.31Z", query: "agence immobiliere", countEst: "18 500+", desc: "Transaction, vente & gestion locative" },
          { id: "promotion_immo", name: "Promotion Immobilière", codeNaf: "41.10A", query: "promoteur immobilier", countEst: "4 200+", desc: "Construction & promotion de logements" },
          { id: "admin_immeubles", name: "Syndics & Admin d'Immeubles", codeNaf: "68.32A", query: "syndic copropriete administrative", countEst: "6 800+", desc: "Gestion de copropriétés & syndics" }
        ]
      },
      {
        category: "Bâtiment, Construction & Artisans (Trades & Construction)",
        icon: "Wrench",
        items: [
          ...
          { id: "electricite", name: "Électricité & Domotique", codeNaf: "43.21A", query: "electricien travaux electrique", countEst: "48 000+", desc: "Installations électriques & réseaux" },
          ...
          { id: "maconnerie", name: "Maçonnerie Générale", codeNaf: "43.99GY", query: "maconnerie travaux publics", countEst: "52 000+", desc: "Gros œuvre & rénovation générale" }
        ]
      },
      {
        category: "Restauration, Hôtellerie & Métiers de Bouche (Food & Hospitality)",
        icon: "Utensils",
        items: [
          ...
          { id: "boulangerie", name: "Boulangerie & Pâtisserie", codeNaf: "10.71C", query: "boulangerie patisserie", countEst: "33 000+", desc: "Artisans boulangers & pâtissiers" }
        ]
      },
      {
        ...
        items: [
          ...
          { id: "dev_logiciel", name: "Programmation & Développeurs", codeNaf: "62.01Z", query: "developpement informatique logiciels web", countEst: "40 000+", desc: "Édition d'applications & sites web" },
          { id: "agences_marketing", name: "Agences Marketing & Pub", codeNaf: "73.11Z", query: "agence de communication marketing", countEst: "22 000+", desc: "Branding, SEO, SEA & publicité" }
        ]
      }
    ]
  },
  ...
  CA: {
    ...
    sources: [
      ...
      {
        name: "Registre des entreprises du Québec (REQ - Données Québec)",
        authority: "Gouvernement du Québec (Revenu Québec)",
        type: "Provincial Business & Enterprise Register",
        provides: ["NEQ (Numéro d'entreprise du Québec)", "Nom & Adresse de siège", "Code d'activité économique (CAE)"],
    ...
    categories: [
      ...
      },
      {
        ...
        icon: "Briefcase",
        items: [
          { id: "ca_accounting", name: "CPA Accounting Firms / Comptables Agréés", codeNaf: "541211", query: "cpa accounting firm cabinet comptable", countEst: "18 000+", desc: "Tax preparation, audit & CFO advisory services" },
    ...
    sources: [
      {
        ...
        authority: "SPF Économie (Gouvernement Fédéral Belge)",
    ...
    categories: [
      {
        ...
        icon: "Wrench",
        items: [
          { id: "be_construction", name: "Entreprises de Construction & Rénovation", codeNaf: "41.20", query: "entreprise de construction renovation batiment", countEst: "28 000+", desc: "Travaux généraux de bâtiment & rénovation" },
          { id: "be_it", name: "Agences IT & Conseil Informatique", codeNaf: "62.01", query: "agence web conseil informatique esn", countEst: "14 000+", desc: "Développement web & services informatiques" }
        ]
      }
    ]
  },
    ...
    sources: [
      {
        ...
        authority: "Office fédéral du registre du commerce (EHRA)",
    ...
    categories: [
      {
        ...
        icon: "Briefcase",
        items: [
          { id: "ch_fiduciaire", name: "Fiduciaires & Conseils Fiscaux", codeNaf: "6920", query: "fiduciaire expert comptable conseil fiscal", countEst: "8 500+", desc: "Fiduciaires suisses, gestion & comptabilité" },
          { id: "ch_tech", name: "Agences Web & IT Solutions", codeNaf: "6201", query: "agence web developpement informatique suisse", countEst: "6 200+", desc: "Développement logiciel & ingénierie suisse" }
        ]
      }
    ]
  },
...
app.get('/api/gouv/explore', async (req, res) => {
  try {
    ...
    if (country === 'CA') {
      ...
      const caFirstNames = ['Jean-François', 'Marc-André', 'David', 'Sarah', 'Alexandre', 'Sophie', 'Patrick', 'Isabelle', 'Michael', 'Catherine'];
      const caLastNames = ['Tremblay', 'Gagnon', 'Roy', 'Côté', 'Bouchard', 'Gauthier', 'Morin', 'Lavoie', 'Fortin', 'Gagné', 'Campbell', 'MacDonald'];
      const caAddresses = [
        { street: '1000 Rue de la Gauchetière O', city: 'Montreal, QC H3B 4W5' },
      ...
      const caLeads = Array.from({ length: perPage }).map((_, idx) => {
        ...
        const suffixes = ['Inc.', 'Ltée / Ltd.', 'Group / Groupe', 'Corporation', 'Associés'];
        ...
        return {
          ...
          name: companyName,
          company: companyName,
          contactName: `${fn} ${ln} (Directeur Général / Director)`,
          phone: '',
          email: null,
    ...
    const countryNames: Record<string, { name: string; cities: { city: string; street: string }[]; regType: string; tld: string }> = {
      ...
      BE: {
        ...
        cities: [
          ...
          { city: '4000 Liège', street: 'Place Saint-Lambert 12' }
        ],
        regType: 'BCE / KBO',
        tld: 'be'
      },
...
app.get('/api/instagram/profile-stats/:username', async (req, res) => {
  try {
    ...
    res.json({
      ...
      bio: `Digital Creator & Influencer ✨ | Daily updates & community 🚀 | Contact via DM 📩`
    });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
...
- "spendText": estimated ad spend (e.g. "€450 - €1,200")
- "adStartDate": active launch date (e.g. "Jan 14, 2026")`;
        ...
        if (Array.isArray(parsed) && parsed.length > 0) {
          aiResearchedAds = parsed.map((item: any, idx: number) => {
            ...
            return {
              ...
              adBody: item.adBody || `👉 Looking for ${keyword}? Special limited offer available today!`,
              ...
              spendText: item.spendText || '€500 - €1,500',
    ...
    const sampleTemplates = [
      {
        ...
        cta: 'Contact Us',
        copySuffix: '⚡ Limited availability this week! Click below for instant free estimate & fast booking.',
      ...
      },
      {
        tag: 'Lead Generation',
        cta: 'Learn More',
        copySuffix: '🔥 Discover how 500+ clients transformed their results with our step-by-step strategy. Free video overview available now!',
      ...
      },
      {
        ...
        cta: 'Shop Now',
        copySuffix: '🎁 Special Offer: 20% OFF your first order with code METASPEC1. Fast shipping & 30-day money-back guarantee!',
      ...
      },
      {
        tag: 'B2B & Agency',
        cta: 'Get Offer',
        copySuffix: '🚀 Stop wasting budget on unvetted leads. Our verified campaign setup generates qualified appointments on autopilot.',
        impressions: '15K - 60K',
        spend: '€450 - €1,100'
      }
    ];
    ...
    for (let i = 0; i < limit; i++) {
      ...
      const adBody = `👉 Looking for top-rated ${keyword}? \n\n${tmpl.copySuffix} \n\n✅ 5-Star Rated Service\n✅ Transparent Pricing & No Hidden Fees\n✅ Fast Response Time`;
  ...
  try {
    ...
    if (isHyperIntent) {
      ...
      if (normalizedMsg.includes('hyperagent') || normalizedMsg.includes('autonomous')) {
        ...
        const respText = result.success
          ? `⚡ **Hyperbrowser AI Stealth Extraction Completed!**\n\n**Target URL:** \`${targetUrl}\`\n\n**Extracted Lead Data:**\n\`\`\`json\n${JSON.stringify(result.data, null, 2)}\n\`\`\``
          : `⚠️ **Hyperbrowser Extract Note:** ${result.error}\n\nMake sure your \`HYPERBROWSER_API_KEY\` is connected in **Settings** tab.`;
...
app.post('/api/leads/clone-site-style', async (req, res) => {
  try {
    ...
    clonedContent.copyrightText = `© ${new Date().getFullYear()} ${companyName}. ${targetLead.city || ''} & Environs. Tous droits réservés.`;
